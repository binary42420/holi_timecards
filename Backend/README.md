...
This folder contains the code for the **Hands on Labor** database application that interacts with a **MySQL** database. Hands on Labor is a single company labor staffing agency based in San Diego, CA.
...
1. **User:** Stores user information (username, password, name, manager status, admin status, active status).
2. **WorkPlace:** DEPRECATED - For single company system, workplace concepts are not used.
3. **UserRequests:** Captures user shift requests.
4. **Shifts:** Represents shifts, including date and time.
5. **ShiftWorkers:** Records shifts assigned to workers.
6. **Jobs:** Represents client jobs and projects.
7. **ClientCompanies:** Stores client company information.
