...
class ShiftBoardService:
    ...
    def publish_board(self, week_start_day, workplace_id: str = None):
        """
        Publish board for Hands on Labor.
        workplace_id parameter is ignored since it's a single company.
        """
        # Get the shiftboard (workplace_id ignored for single company)
        board = self.repository.get_entity(week_start_day, workplace_id)

        # Check if the shiftboard is already published
        if board.published:
            raise ValueError(f"Board starts at {week_start_day} for Hands on Labor already published")
